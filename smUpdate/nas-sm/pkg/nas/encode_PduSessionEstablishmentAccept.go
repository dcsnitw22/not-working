...
func EncodePduSessionEstablishmentAccept(pdu PduSessionEstablishmentAccept) ([]byte, error) {
	...
	//Next Octets are SM Cause - But optional

	//Next Octets are pdu session address
	pduSesAddr, err := EncodePduSessionAddress(pdu.Si6lla, pdu.PduSessionTypeVal, pdu.PduAddrInfo)
	if err != nil {
		return nil, err
	}
	byteArray = append(byteArray, pduSesAddr...)
