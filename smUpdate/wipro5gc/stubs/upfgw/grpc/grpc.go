...
func (g *GrpcInfo) Start() {
	...
	klog.Info("Sending Tunnel data to SMF")
	...
	go func() {
		for {
			...
			case ch := <-g.WatchGrpcChannel():
				ids := (*ch.GrpcMsg).(grpcserver.Ids)
				tunnelId := GetRandomID()
				tunnelIP := GetRandomIP()

				// Stub dummy data
				n1n2Data := protos.N1N2MessageTransferDataRequest{
					SmContextID:       ids.SmContextID,
					UeContextID:       ids.UeContextID,
					NgapleType:        "PDU_RES_SETUP_REQ",
					TunnelEndpointAdr: tunnelIP,
					Gtpteid:           tunnelId,
				...
				g.SendN1N2MessageData(ids.SmContextID, &n1n2Data)
			}
		}
	}()
}
