...
func SendPduSessionResourceSetupRequest(ran *context.AmfRan) error {
	n1 := <-apiserver.N1Msg
	n2 := <-apiserver.N2Info
	n2d := &asn1gen.PDUSessionResourceSetupRequestTransfer{}
	_, err := asn1gen.Unmarshal(n2, n2d)
	if err != nil {
		return err
	}
	//AMF UE NGAP ID
...
func HandlePduSessionResourceReleaseResponse(ran *context.AmfRan, message *asn1gen.NGAPPDU) {
	...
	for i := 0; i < len(pduSessionResourceReleaseResponseIEs); i++ {
		...
		case asn1gen.Asn1vIdPDUSessionResourceReleasedListRelRes:
			...
			if err != nil {
				logger.AppLog.Error("unmarshaling of PDU Session Resource Released List failed")
				return
			}
			if len(pduSessionResourceReleasedList) == 0 {
				logger.AppLog.Error("pduSessionResourceReleasedList is empty")
				return
			}
