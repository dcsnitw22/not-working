...
func (p *PdusmspSessionWorkers) HandleSessionMessages(msgInfo *SessionMessageInfo) {
	...
	if sessionWorker, exists = p.sessionWorker[sessionId]; !exists {
		...
		// Create a new worker for new session or when smf-upfgw restarted.
		go func() {
			//defer ??
	...
	klog.Info(msgInfo.MsgType)
}

// sessionWorkerHandler starts a new worker for a new session to handle its messages
func (p *PdusmspSessionWorkers) sessionWorkerHandler(sessionMessages <-chan SessionMessageInfo) {
...
func (p *PdusmspSessionWorkers) RemoveSessionWorkers(deletedSessions map[SessionId]struct{}) error {
	p.sessionLock.Lock()
	defer p.sessionLock.Unlock()
