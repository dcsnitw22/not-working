...
func (g *GrpcClient) Start() {
	...
	conn, err := grpc.Dial(g.ConnAddrUpfgw, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
	...
	conn, err = grpc.Dial(g.ConnAddrNgap, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		klog.Fatalf("Dial error. Did not connect: %v", err)
	}
...
func (g *GrpcClient) SendSmContextCreateData(createData *protos.SmContextCreateDataRequest) {
	...
	klog.Infof("Response from server: %v", r.GetSmContextID())
}
...
func (g *GrpcClient) SendSmContextUpdateData(updateData *protos.SmContextUpdateDataRequest) {
	...
	klog.Infof("Response from server for update/release: %v", res)
}
