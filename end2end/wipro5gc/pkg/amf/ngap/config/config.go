...
type NgapConfig struct {
	...
	GrpcClientInfo GrpcClientInfoConfig //for nasmm - to forward nasPdu and other data
	GrpcServerInfo GrpcServerInfoConfig //for pdusmsp - to NGAP encode n2info, for nasmm - to receive n1n2 message
	DbInfo         DbConfig
}
...
var defaultNgapConfig = []byte(`
{
		...
		"GrpcServerInfo":{
			"ServerIPPdusmsp": "0.0.0.0",
			"ServerPortPdusmsp": "50055",
			"ServerIPNas": "0.0.0.0",
			"ServerPortNas": "50056"
		},
		"DbInfo":{
			"Redis": {
				"IP": "redis.redis.svc.cluster.local",
				"Port": "6379",
				"Pass": "redisWipro",
				"Db": 2
			}
...
func InitConfig(cfgFile string, etcdServer string, etcdConfigKey string, resetFlag bool) (*NgapConfig, error) {
	...
	if !etcdConfig {
		...
		if err := runtime_viper.ReadInConfig(); err != nil {
			...
			klog.Info("Successfully read config file with default values")
		}
